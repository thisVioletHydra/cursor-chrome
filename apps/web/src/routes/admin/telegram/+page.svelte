...
<header class="mb-5 flex items-end justify-between gap-4">
  ...
  {#if link?.ok}
    <button class="btn btn-error btn-sm" type="button" onclick={() => ask = true}>Отвязать</button>
  {/if}
</header>

{#if link?.ok && username}
  <a class="flex items-center gap-4 rounded-2xl border border-white/8 bg-[#151922] px-4 py-3" href="https://t.me/{username}" target="_blank" rel="noreferrer">
    ...
    <span class="ml-auto text-sm text-indigo-300 underline-offset-4 hover:underline">@{username}</span>
  </a>
